    ...
    return (
        <Link to={`/product/${product.id}`} className="block h-full outline-none">
            <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] hover:shadow-[0_8px_30px_-4px_rgba(6,81,237,0.12)] transition-shadow duration-300 border border-slate-100 flex flex-col h-full group cursor-pointer relative z-0">
                {/* Tag */}
                <h4
                    className={`text-xs font-bold tracking-wider uppercase mb-2 ${product.tagColor === "purple" ? "text-purple-500" : "text-blue-500"
                        }`}
                >
                    {product.tag}
                </h4>
                ...
                <div className="flex flex-col flex-grow">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-1">
                        ...
                        <button
                            className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-medium text-sm px-5 py-2 rounded-full shadow-md shadow-blue-500/20 hover:shadow-lg hover:shadow-blue-500/30 transition-all active:scale-95 relative z-10"
                            onClick={(e) => {
